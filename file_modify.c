...
int modify_bmp(unsigned char *f, int file_size) {
    ...
    if ((p_bmp_head->bfSize - p_bmp_head->bfOffBits) != p_bmp_head->biSizeImage) {
        printf("BMP size abnormal, biSizeImage: %u, (bfSize - bfOffBits): %u\r\n", p_bmp_head->biSizeImage, p_bmp_head->bfSize - p_bmp_head->bfOffBits);
    }
    ...
    trueheight = (p_bmp_head->biSizeImage / bitcount) / p_bmp_head->biWidth;
